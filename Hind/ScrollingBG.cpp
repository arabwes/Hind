...
void Scrolling_BG::InitBG(LPDIRECT3DDEVICE9 d3ddev, int SCREENW, int SCREENH, std::string filename)
{
	...
	LPDIRECT3DSURFACE9 tImage = LoadSurface(filename);
	...
	d3ddev->StretchRect(tImage, &SOURCE_RECT, ScrollBG, &src_ul, D3DTEXF_NONE);

	RECT src_ur = { SCREENW, 0, SCREENW * 2, SCREENH };
	d3ddev->StretchRect(tImage, &SOURCE_RECT, ScrollBG, &src_ur, D3DTEXF_NONE);

	RECT src_ll = { 0, SCREENH, SCREENW, SCREENH *2 };
	d3ddev->StretchRect(tImage, &SOURCE_RECT, ScrollBG, &src_ll, D3DTEXF_NONE);

	RECT src_lr = { SCREENW, SCREENH, SCREENW * 2, SCREENH * 2 };
	d3ddev->StretchRect(tImage, &SOURCE_RECT, ScrollBG, &src_lr, D3DTEXF_NONE);
